document.addEventListener('DOMContentLoaded', () => {
    ...
    // const customerListDiv = document.getElementById('customer-list'); // Non più usato per la lista principale
    ...
    // NUOVI ELEMENTI PER LA GESTIONE CLIENTI CON RICERCA
    const searchManageCustomersInput = document.getElementById('search-manage-customers');
    const manageCustomerListResultsDiv = document.getElementById('manage-customer-list-results'); // Qui verranno mostrati i clienti nella sezione di gestione
    ...
    manageCustomersBtn.addEventListener('click', () => {
        searchManageCustomersInput.value = ''; // Pulisci la barra di ricerca
        displayCustomers(); // Mostra tutti i clienti quando si entra nella sezione
    ...
    // Funzione per visualizzare i clienti nella sezione "Gestisci Clienti"
    // Ora accetta un array opzionale di clienti da visualizzare
    function displayCustomers(customersToDisplay = customers) {
        manageCustomerListResultsDiv.innerHTML = ''; // Pulisci i risultati precedenti
        if (customersToDisplay.length === 0) {
            manageCustomerListResultsDiv.textContent = 'Nessun cliente trovato.';
        } else {
            customersToDisplay.sort((a, b) => a.name.localeCompare(b.name)); // Ordina per nome
            ...
            customersToDisplay.forEach(customer => {
                ...
                `;
                manageCustomerListResultsDiv.appendChild(customerDiv);
            });
        }
    }

    // Event listener per la barra di ricerca nella sezione "Gestisci Clienti"
    searchManageCustomersInput.addEventListener('input', () => {
        const searchTerm = searchManageCustomersInput.value.toLowerCase().trim();
        if (searchTerm === '') {
            displayCustomers(); // Se la ricerca è vuota, mostra tutti i clienti
        } else {
            const filteredCustomers = customers.filter(customer =>
                customer.name.toLowerCase().includes(searchTerm) ||
                customer.street.toLowerCase().includes(searchTerm)
            );
            displayCustomers(filteredCustomers); // Mostra solo i clienti filtrati
        }
    });

    // Event listener per i pulsanti Modifica ed Elimina (delegazione eventi)
    // Ora ascolta sul div manageCustomerListResultsDiv
    manageCustomerListResultsDiv.addEventListener('click', (event) => {
        ...
        if (currentEditingCustomerId) {
            ...
            if (index !== -1) {
                ...
                if (updatedName && updatedStreet) {
                    ...
                    editCustomerModal.classList.add('hidden');
                    displayCustomers(); // Aggiorna la lista dopo la modifica
                    searchManageCustomersInput.dispatchEvent(new Event('input')); // Rilancia la ricerca per aggiornare i risultati se c'è un termine di ricerca
                } else {
    ...
    function deleteCustomer(customerId) {
        if (confirm('Sei sicuro di voler eliminare questo cliente? Questa operazione non può essere annullata.')) {
            ...
            displayCustomers(); // Aggiorna la lista dopo l'eliminazione
            searchManageCustomersInput.dispatchEvent(new Event('input')); // Rilancia la ricerca per aggiornare i risultati se c'è un termine di ricerca
        }
    }
});
